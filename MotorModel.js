...
// Значения паспорта по умолчанию для тех четырёх величин, которые меняются из
// панели. Собраны отдельно, потому что нужны дважды: при создании машины и при
// сбросе, который возвращает их на место вместе с остальными параметрами
// интерфейса. Остальной паспорт задаётся прямо в полях класса — его никто не
// меняет, и повторять его здесь незачем.
const DEFAULT_STATOR_RESISTANCE = 1.2;
const DEFAULT_STATOR_INDUCTANCE = 0.006;
const DEFAULT_MAGNET_FLUX = 1.25;
const DEFAULT_INERTIA = 0.1;

// Паспорт машины. Четыре величины — сопротивление обмотки, её индуктивность,
// потокосцепление магнитов и момент инерции — настраиваются из панели: от них
// зависят постоянные времени, моментная постоянная и противо-ЭДС, которые и
// показывает модель. Всё остальное здесь зафиксировано в
// коде: задания и коэффициенты регуляторов живут в ControlSettings, а паспорт
// меняется только карточкой «Параметры двигателя».
//
// Объект паспорта один на всю программу: его держат модель, регуляторы и
// отрисовка, и все читают его поля при каждом обращении. Поэтому изменение
// ползунком действует со следующего же шага — пересобирать ничего не нужно.
class MotorParameters {
  // Электрическая часть. Сопротивление и индуктивность вместе задают
  // постоянную времени обмотки L/R: при значениях по умолчанию это 5 мс.
  // ψf = 1,25 Вб при p = 1 даёт моментную постоянную 1,5·p·ψf = 1,875 Н·м/А;
  // та же ψf задаёт противо-ЭДС ψf·ωэ, а с ней и скорость, на которой
  // напряжения инвертора перестаёт хватать.
  statorResistance = DEFAULT_STATOR_RESISTANCE;
  statorInductance = DEFAULT_STATOR_INDUCTANCE;
  magnetFlux = DEFAULT_MAGNET_FLUX;
  polePairs = 1;

  // Ld и Lq — это одна и та же индуктивность: машина неявнополюсная, и
  // реактивный момент из формулы момента при этом обращается в нуль, а момент
  // оказывается пропорционален одному iq. Обе оси взяты от одного поля, а не
  // от двух: ползунок в панели один, и разойтись им теперь нечем. Явнополюсной
  // машине здесь понадобились бы два независимых поля — уравнения в модели и в
  // регуляторах для этого уже записаны через Ld и Lq по отдельности.
  get inductanceD() {
    return this.statorInductance;
  }
  ...
  // времени J/Bv при значениях по умолчанию много больше электрической,
  // поэтому скорость меняется заметно медленнее тока — это видно и на экране.
  ...
  // Возврат настраиваемой части паспорта к значениям по умолчанию. Вызывается
  // сбросом наравне с resetGuiParametersPreservingMode: раз эти четыре величины
  // задаются из панели, они такой же параметр интерфейса, как коэффициенты
  // регуляторов, и кнопка «Сброс» возвращает их вместе с ними.
  resetTunableParameters() {
    this.statorResistance = DEFAULT_STATOR_RESISTANCE;
    this.statorInductance = DEFAULT_STATOR_INDUCTANCE;
    this.magnetFlux = DEFAULT_MAGNET_FLUX;
